...
class OwnersController extends CrudController
{
	public $tpl_dir = 'modules/apps/admin/owners';
	...
	function __construct()
	{
		...
		->addExclusion(['email', 'image', 'mobile', 'address'], Tableci::HTML_DATATABLE);
	}
	public function pre_index(&$table)
	{
		$table = $this->ctrl_model::with('rel_user_extension')
		...
		$table->each(function($item, $key)
		{
			...
			$item->makeHidden(['reference']);
 
			$item->email = $item->rel_user_extension ? $item->rel_user_extension->email : null;

			$item->image = $item->rel_user_extension ? $item->rel_user_extension->img_filename : null;

			$item->mobile = $item->rel_user_extension ? $item->rel_user_extension->mobile : null;

			$item->address = $item->rel_user_extension ? $item->rel_user_extension->address : null;

			if ($item->image)
			{
				$url = base_url('public/uploads/'.$item->image);

				$img = HtmlElement::render('img.img-preview.rounded', [
					'src' => $url
				], '');

				$item->image = HtmlElement::render('a', [
					'href' => $url,
					'data-popup' => 'lightbox'
				], $img);
			}
