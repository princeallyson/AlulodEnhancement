...
/*
    Github Link: https://gist.github.com/asaokamei/3747335
 */
...
class Tagsci
{
    ...
    public function __call( $name, $args )
    {
        if (isset($this->helpers[$name])) {
            return $this->helpers[$name]->$name($this, ...$args);
        }
     ...
     * @return Tags
     */
    protected function setContents_( $contents, $index=null ) {
        ...
        $this->contents = array_values(array_filter($this->contents));
    ...
    {
        return $this->setContents_($content, 0);
    }
    ...
    {
        return $this->setAttribute_($name, $value);
    }
     ...
     */
    public function getTags($index=null)
    {
        return !is_null($index) ? ($this->contents[$index] ?? null) : $this->contents;
    }
    ...
    public function getLast()
    {
        ...
        return $this->getTags(count($this->contents) -1);
    }
    public function getInstance($new_instance=false)
    {
        return $new_instance ? new self() : $this;
    }
    public function addContents($contents, $index=null)
    {
        if (is_array($contents)) {
            foreach ($contents as $content) {
                $content = getClass($content) == 'Template' ? $content->getFirst() : $content;
                
                $this->setContents_($contents, $index);                
            }

            return $this;
        }
        else
            return $this->setContents_($contents, $index);
    }
    public function minify()
    {   
        return html_minify($this->__toString());
    }
    public function replaceClass($old_class, $new_class)
    {
        if (!empty(findItem($this->attributes, 'class')))
            $this->attributes['class'] = str_replace(trim($old_class), trim($new_class), findItem($this->attributes, 'class'));
    ...
    {
        return strContains(findItem($this->attributes, 'class'), $class);
    }
